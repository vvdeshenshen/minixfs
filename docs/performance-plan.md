...
| `CPU` 加 `__slots__`(属性走槽描述符) | 子类原型, gzip 实测 | 1.00x(1.96/2.03s 对 1.97/2.05s) | CPython 3.12 对实例字典属性已有内联缓存特化(LOAD_ATTR_INSTANCE_VALUE), 与槽同速; 微基准里的差距是 timeit 热身效应。**无收益** |
| flags 搬进 `regs[8]`(ALU handler 少两次属性访问) | 模板替换 + property 原型, gzip 实测 | 0.984x | 模板路径省下的两次属性访问只有 ~10ns, 而 jcc 多一次 `cpu.regs` 读、非模板路径(hlt/clc/int 后内核、快照)经 property 变慢, 得不偿失。**放弃** |
