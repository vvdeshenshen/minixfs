...
寄存器有两套视图:
- **名字视图**:`cpu.eax` 是 8 个 `property`(`_reg_property`,cpu86.py),映射到
...
放大到数千万倍(这正是性能剖析要做成两档、默认关的原因)。热路径上已做的微优化
与后续阶段(查表分派、解码缓存)的方案见 `docs/performance-plan.md`。
...
- **线性 if 链 vs 跳转表**:线性链好读、易对照 Intel 手册分段,但每条指令平均要过
  若干个 `if`。第一阶段微优化之后,cProfile 显示这条 if 链的自身开销已占约 30%,
  成为最大热点;下一步是"解码/执行分离 + 256 项分派表",再在其上做解码缓存
  (把 eip→已解码元组记下来),详见 `docs/performance-plan.md`。
- **热路径纪律**(已实测的教训):不要给 `CPU` 加 `__setattr__/__getattr__` 之类的
  钩子;访存用 `unpack_from/pack_into` 直读 bytearray,不要 `bytes(切片)` 再 `unpack`;
  按尺寸取 mask/符号位用模块级元组 `_MASK/_SIGN` 查表,不要算式或方法调用;
  取指/ModRM 这类每条必走的代码把 `eip/mem/regs` 放局部变量、末尾只写回一次。
